...
const AgencyPackages = () => {
    ...
    const loadAgencyData = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          console.log('✅ Agency packages loaded:', response.data.packages?.length || 0);
        } else {
          throw new Error(response.data.message || 'Agency not found');
        }
      } catch (error) {
        console.error('❌ Error loading agency packages:', error);
        setError(error.response?.data?.message || 'Failed to load agency details');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredPackages = packages.filter(pkg => {
    ...
    if (priceFilter !== 'all') {
      ...
      if (priceFilter === 'mid' && (pkg.price < 1000 || pkg.price > 3000)) matchesPrice = false;
    ...
    if (durationFilter !== 'all') {
      ...
      if (durationFilter === 'medium' && (days < 5 || days > 10)) matchesDuration = false;
    ...
    return (
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Agency Not Found</h3>
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Filter Packages</h2>
            ...
            <div key={pkg._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-6">
                ...
                <div className="mb-4 space-y-2 text-sm text-gray-600">
                  ...
                  <p className="flex items-center">
                    <span className="mr-2">📅</span>
                    <span>{pkg.durationDays || pkg.durationMinutes} {pkg.durationDays ? 'days' : 'minutes'}</span>
                  </p>
                  {pkg.difficulty && (
                    <p className="flex items-center">
                      <span className="mr-2">⚡</span>
